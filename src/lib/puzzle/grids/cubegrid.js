...
export class CubeGrid {
	...
	constructor(width, height, wrap, tiles = []) {
		...
		this.lineJoin = iOS() ? 'miter-clip' : 'bevel';
	...
	detectEdgemarkGesture(tile_index, tile_x, tile_y, x1, x2, y1, y2) {
		return this.polygon_at(tile_index).detect_edgemark_gesture(tile_x, tile_y, x1, x2, y1, y2);
	}
	 ...
	 */
	whichEdge(point) {
		return this.polygon_at(point.tileIndex).is_close_to_edge(point);
	}
}
